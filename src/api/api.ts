...
export default class Api {
    ...
    communtityApi = process.env.VUE_APP_COMMUNITY_API;
    ...
    async imageUplaod(src: string) {
        ...
        const response = await this.request('post', '/community/att/i', formData, false);
    ...
    post = {
        //todo: 확인중
        upload: async (obj: any) => {
            return await this.request('post', `${this.communtityApi}post`, obj, false);
        },

        read: async (post_id: string) => {
            return await this.request('get', `${this.communtityApi}post/${post_id}`, undefined, false)
        },

        like: async (post_id: string) => {
            return await this.request('post', `${this.communtityApi}post/${post_id}/like`, undefined, false)
        },
        likeList: async (post_id: string) => {
            return await this.request('get', `${this.communtityApi}post/${post_id}/like/list`, undefined, false)
        }

    }

    comment = {
        upload: async (obj: { user_id: number, parent_id: string | null, post_id: string, content: string, is_private: boolean, type: string, attatchment_files?: [{ priority: number, url: string }] }) => {
            return await this.request('post', `${this.communtityApi}post/${obj.post_id}/comment`, obj, false)
        },
        list: async (post_id: string) => {
            return await this.request('get', `${this.communtityApi}post/${post_id}/comment/list`, undefined, false)
        },
        update: async (comment_id: string, obj: { user_id: number, parent_id: string | null, post_id: string, content: string, is_private: boolean, type: string, attatchment_files?: [{ priority: number, url: string }] }) => {
            return await this.request('post', `${this.communtityApi}post/${obj.post_id}/comment/${comment_id}`, obj, false)
        },
        delete: async (post_id: string, comment_id: string) => {
            return await this.request('delete', `${this.communtityApi}post/${post_id}/comment/${comment_id}`, undefined, false)
        },
        like: async (post_id: string, comment_id: string,) => {
            return await this.request('post', `${this.communtityApi}post/${post_id}/comment/${comment_id}/like`, undefined, false)
        }
    ...
    group = {
        ...
        create: async (user_uid: string, name: string, url: string, description: string, is_private: boolean, profile_img?: string, banner_img?: string) => {
            const formData = new FormData();

            if (user_uid) { formData.append('user_uid', user_uid.toString()); }
            if (name) { formData.append('community_name', name); }
            if (url) { formData.append('community_url', url); }
            if (description) { formData.append('community_desc', description); }
            // formData.append('is_private', is_private.toString());
            if (profile_img) { formData.append('community_profile_img', 'https://contattafiles.s3.us-west-1.amazonaws.com/tnt34018/jORm1SBLuQbYoYl/iconfinder_Piggy_Bank_7013430.png'); }
            if (banner_img) { formData.append('community_banner_img', 'https://contattafiles.s3.us-west-1.amazonaws.com/tnt34018/6xtY14ePjEbTzjw/iconfinder_Brother_3231124.png'); }
            ...
            return await this.request('post', `${this.communtityApi}community`, formData, false)
        },
        delete: async (group_id: string) => {
            return await this.request('post', `${this.communtityApi}community/${group_id}/remove`, undefined, false)
        },

        // update: async 
        list: async (sort?: string, limit?: number, offset?: number) => {
            //  throw Error;
            return await this.request('get', `${this.communtityApi}community/list`, { sort }, false)
        },
        info: async (id: string) => {
            return await this.request('get', `${this.communtityApi}community/${id}`, undefined, false)
        },
        members: async (id: string) => {
            return await this.request('get', `${this.communtityApi}community/${id}/members`, undefined, false)
        },
        subscribe: async (user_id: number, group_id: string) => {
            return await this.request('post', `${this.communtityApi}community/${group_id}/subscribe?user_id=${user_id}`, undefined, false)
        },
        channel: {
            info: async () => {
            ...
            list: async (group_id: string) => {
                return await this.request('get', `${this.communtityApi}community/${group_id}/channels`, undefined, false);
            },
            delete: async (group_id: string, channel_id: string) => {
                return await this.request('post', `${this.communtityApi}community/${group_id}/channel/${channel_id}/remove`, undefined, false)
            }
    ...
    user = {
        follow: async (user_id: number) => {
            return await this.request('post', `${this.communtityApi}user/${user_id}/follow`, undefined, false)
        },
        unfollow: async (user_id: number) => {
            return await this.request('post', `${this.communtityApi}user/${user_id}/unfollow`, undefined, false)
        },
        groupList: async (user_id: number, sort?: string, limit?: number, offset?: number) => {
            return await this.request('get', `${this.communtityApi}user/${user_id}/list/community`, undefined, false)
        },
        mine: async () => {
            return await this.request('get', `${this.communtityApi}user/mine`, undefined, false);
        },
        followingList: async (user_id: number, limit?: number, offset?: number) => {
            return await this.request('get', `${this.communtityApi}user/${user_id}/list/following`, undefined, false)
        },
        followerList: async (user_id: number, limit?: number, offset?: number) => {
            return await this.request('get', `${this.communtityApi}user/${user_id}/list/follower`, undefined, false)
        }
    ...
    async search(query: string, type?: string, sort?: string) {
        ...
        let result: any;
        if (type === 'community') {
            return await this.request('get', `${this.communtityApi}search`, { query, type }, false)
            result = [{
                "id": 1,
    ...
    async createCommunity(user_uid: string, name: string, url: string, description: string, is_private: boolean, profile_img?: string, banner_img?: string) {
        ...
        if (description) { formData.append('community_desc', description); }
        // formData.append('is_private', is_private.toString());
        if (profile_img) { formData.append('community_profile_img', profile_img); }
        if (banner_img) { formData.append('community_banner_img', banner_img); }
        ...
        const response = await axios.post(`${this.communtityApi}community`, formData, config)

        // const response = await this.request('post', `${this.communtityApi}community`, formData, false);
        console.log(response)
    ...
    getTimeline(id: number | string, channelId?: number, portfolioId?: number) {
        ...
        if (id && portfolioId) {
            ...
            result = [
                {
                    "id": 'c90039a7-4ed8-4f9e-8f95-29e3193c24e9',
                    "user": {
                        "id": 1,
                        "uid": 'wjZpvIjDEMWUBdXKsUQyR33RWrx2',
                        "status": "owner ",
                    ...
                    "content": "<p>안녕하세요. 포트폴리오 타임라인 </p>",
                    "is_private": true,
                    "hashtags": ["porfolio1", "porfolio1"],
                    "user_tag": [],
                    "liked": true,
                    "like_cnt": 1,
                    "comment_cnt": 5,
                ...
                },
                {
                    ...
                    "content": `<p><span data-mention="" class="mention" data-id="following1" channel-id="wjZpvIjDEMWUBdXKsUQyR33RWrx2">@following1</span> <span data-hashtag="" class="hashtag" data-id="hashtag8">#hashtag8</span> 포트폴리오 예시 피드</p>`,
                    "is_private": false,
                    "hashtags": ["tag0", "tag1", "tag2"],
                    "user_tag": [],
                    "liked": false,
    ...
    async follow(user_uid: string) {
        const response = this.request('post', `${this.communtityApi}member/2/follow`, undefined, false)
        return true;
        // let result=
    ...
    async uploadpost(user_uid: string, post_type: string, visibility: string, fileList: { img: fileObjWtUrl[], video: fileObjWtUrl[], audio: fileObjWtUrl[] }, is_private: boolean, content: string, hashtags?: string[], userTags?: string[], community?: any[], communityId?: number, channelId?: number, gameId?: number, portfolioId?: number, scheduled_for?: number) {
        const formData = new FormData();

        if (user_uid) { formData.append('user_uid', user_uid); }
        if (post_type) { formData.append('post_state', post_type.toUpperCase()); }
        ...
        if (communityId) { formData.append('community_id', user_uid); }
        ...
        const result = this.request('post', `${this.communtityApi}post`, formData, false);
        // console.log(result)
    ...
    async user1() {
        return this.request('get', '/user/info', undefined, true);
