...
export default class Api {
    ...
    post = {
        //todo: 확인중
        upload: async (user_uid: string, post_type: string, visibility: string, fileList: { img: fileObjWtUrl[], video: fileObjWtUrl[], audio: fileObjWtUrl[] }, is_private: boolean, content: string, hashtags?: string[], userTags?: string[], community?: any[], communityId?: number, channelId?: number, gameId?: number, portfolioId?: number, scheduled_for?: number) => {
            ...
            if (userTags) {formData.append('userTags', JSON.stringify(userTags));}
            if (community) {
    ...
    group = {
        ...
        subscribe: async (user_id: number, community_id: string) => {
            return await this.request('post', `${this.communtityApi}community/${community_id}/subscribe?user_id=${user_id}`, undefined, false)
        }
